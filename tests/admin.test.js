...
describe('Admin override Transaction,', () => {
    ...
    beforeEach(async () => {
        ...
        const user = await User.create({ username: 'normaluser', password:
         ...
         adminToken = jwt.sign({ id: admin_.id, role: admin.role},
            process.env.JWT_SECRET);
         normalToken = jwt.sign({ id: admin_.id, role: admin.role},
            process.env.JWT_SECRET);
        ...
        it('should prevent non-admin from overriding a transaction', async() => {
            ...
            expect(res.body.message).toBe('Forbidden: Insuficient role permissions');
